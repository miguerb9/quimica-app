...
export default function NoteForm({ note, subjects }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (pdfFile && saved.id) {
      await uploadPdf(pdfFile, saved.id)
    }
  ...
  async function uploadPdf(file: File, noteId: string) {
    setPdfLoading(true)
  ...
  async function handleRemovePdf() {
    ...
    setPdfLoading(true)
    const res = await fetch(`/api/admin/upload-pdf?noteId=${savedNoteId}`, { method: 'DELETE' })
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-5">
      ...
      <div>
        <label className="block text-sm font-medium text-ink mb-1.5">
          PDF adjunto <span className="text-muted font-normal text-xs">(opcional, máx. 20 MB)</span>
        </label>
        {pdfUrl ? (
      ...
      <div className="flex gap-3 pt-2">
        <button type="submit" disabled={loading || pdfLoading} className="bg-accent text-white px-5 py-2 rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50 transition-colors">
          {loading ? 'Guardando...' : note ? 'Guardar cambios' : 'Crear apunte'}
        </button>
