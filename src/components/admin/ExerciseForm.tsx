...
import type { Exercise, Asignatura } from '@/types'
...
export default function ExerciseForm({ exercise, subjects }: Props) {
  ...
  const initialAsignatura: Asignatura =
    subjects.find(s => s.id === exercise?.subject_id)?.asignatura ?? 'quimica'
  ...
  const [form, setForm] = useState({
    subject_id:    exercise?.subject_id    ?? filteredSubjects[0]?.id ?? '',
  ...
  const [pdfUrl, setPdfUrl] = useState<string | null>(exercise?.pdf_url ?? null)
  const [pdfLoading, setPdfLoading] = useState(false)
  const [savedExerciseId, setSavedExerciseId] = useState<string | null>(exercise?.id ?? null)
  ...
  async function handleRemovePdf() {
    ...
    setPdfLoading(true)
    const res = await fetch(`/api/admin/upload-pdf?exerciseId=${savedExerciseId}`, { method: 'DELETE' })
    if (res.ok) setPdfUrl(null)
    else setError('No se pudo eliminar el archivo')
    setPdfLoading(false)
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-5">
      ...
      <div>
        <label className="block text-sm font-medium text-ink mb-1.5">
          PDF del ejercicio <span className="text-muted font-normal text-xs">(opcional, máx. 20 MB)</span>
        </label>
        {pdfUrl ? (
          <div className="flex items-center gap-3 p-3 border border-border rounded-lg bg-white">
            <span className="text-red-500 text-xl">📄</span>
            <a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-accent hover:underline flex-1 truncate">Ver PDF actual</a>
            <button type="button" onClick={handleRemovePdf} disabled={pdfLoading} className="text-xs text-danger hover:underline disabled:opacity-50">
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <label htmlFor="show_solution" className="text-sm font-medium text-ink">Mostrar solución a los alumnos</label>
      </div>

      {error && <p className="text-danger text-sm bg-red-50 border border-red-200 rounded-lg px-3 py-2">{error}</p>}

      <div className="flex gap-3 pt-2">
        <button type="submit" disabled={loading || pdfLoading} className="bg-accent text-white px-5 py-2 rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50 transition-colors">
